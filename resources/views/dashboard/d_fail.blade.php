            ...
            <div class="container-fluid">
                <section class="py-2">
                    <div class="bg-white border rounded border-info rounded-4 pt-4">
                        <h4 class="text-primary mb-3 ps-4">Displays Not Ok</h4>
                        <div class="table-responsive rounded-4">
                            @if(count($d_fail_recent)==0)
                            <hr class="m-0">
...
<h6 class="mt-3">Nothing to check here!</h6>
                            ...
                            <table class="table mb-0 table-light">
                                ...
                                <tbody>
                                    @foreach($d_fail_recent as $df)
                                    <tr>
                                        <td class="text-primary">{{$df->manufacturer}} {{$df->model}} ({{$df->serial}})</td>
                                       
                                        <td class="text-body">
                                        @if($df->workstation->workgroup->address!=NULL)
                                        {{$df->workstation->workgroup->address}}
                                        
                                        @if($df->workstation->workgroup->city!=NULL)
                                        , {{$df->workstation->workgroup->city}}
                                        @endif
                                        
                                        @if($df->workstation->workgroup->state!=NULL)
                                        , {{$df->workstation->workgroup->state}}
                                         @endif
                                            
                                        @endif
                                        </td>
                                        <td class="text-danger">
                                             @if ($errors = json_decode($df->errors, true))
                                        ...
                                        <td class="text-body">{{date_format(new DateTime($df->updated_at), 'd/m/Y, h:i A')}}</td>
                                        <td>
                                            <div class="dropdown">
...
<path d="M7.5 11.5H12.5" stroke="#EB5757" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                ...
                                <tfoot>
                                    <tr>
                                        <td class="text-body" colspan="3">Showing {{count($d_fail_recent)}} of {{$d_fail}} entries</td>
                                        <td colspan="2">
                                            <nav>
