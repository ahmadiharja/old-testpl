...
class settings extends Controller
{
    ...
    {
        return view('settings.build_version', ['title'=>'Build New Version']);
    }
    ...
    public function alert_settings(Request $request)
    {
        ...
        $alert->send_after_task = $request->input('send_after_task') ? 1 : 0;
        ...
        return view('settings.alert_settings', ['title' => 'Alert Settings'])->with($data);
    }
    ...
    public function list_alerts(Request $request)
    {
        $user_id=$request->session()->get('id');
        $user=\App\Models\User::find($user_id);
        ...
        $isSuper = 0;
        if($role=='super') $isSuper = 1;
        $facility_id=$user->facility_id;
        
        $alert = \App\Models\Alert::With('Facility')
            ->when(!$isSuper, function ($q) use($user) {
                return $q->where('facility_id', $user->facility_id);
            })
            ->latest()->get();
        // print_r($alert);
        // die('ddd');
    ...
    public function form(Request $request)
    {
       ...
       if(!isset($alert->id)){
           $alert=\App\Models\Alert::limit(1)->get();
           $alert->id=0;
           $alert->email='';
           $alert->actived='1';
           $alert->daily_report='1';
           $alert->send_after_task='0';
           $alert->facility_id=0;
       }
        $data['success']=1;
    ...
    public function global_settings(Request $request)
    {
        ...
        return view('settings.global_settings', ['title' => 'Global Settings', 'facilities'=>$facilities, 'user_workstations'=>$user_workstations]);
    }
