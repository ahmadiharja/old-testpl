    ...
    private function HISTORY()
    {
        ...
        foreach ($this->req_data as $history) {
            ...
            if (!$newHist) {
                ...
                // Only send for NEW records to prevent duplicate emails if client resends same history
                if (in_array($history['result'], [2, 3]) && !config('app.offline')) {
                    try {
                        $alerts = Alert::where([['facility_id', $this->facility->id], ['actived', 1], ['send_after_task', 1]])->get();
                        foreach ($alerts as $alert) {
